...
These are the decisions a naive reimplementation is most likely to get wrong,
and why the implementation made them.
...
- `tests/conformance/*.conformance.ts` pins the wire/parse/format behavior and
  can be run against any implementation via `PI_WEB_SEARCH_IMPL`.

**When you change observable behavior, update the SPEC, the fixtures, and the
conformance suite in the same change.** If you don't, `npm run verify` will tell
