...
- `isRetryableDuckDuckGoError` returns `false` for
  ...
  including Node's `execFile` abort (`name === "AbortError"` or
  `code === "ABORT_ERR"`), not only `DOMException`. It returns `true` for all
  other errors (transient I/O). A DuckDuckGo request that hits the 15 s timeout
  therefore fails fast instead of retrying.
