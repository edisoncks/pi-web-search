...
| **Audience** | Anyone reimplementing this extension from documentation alone |
...
> written *as implemented at v1.0.0*. The committed fixtures and the
> conformance suite (see §12) are the executable truth; if they disagree with
...
Additional literals pinned by the conformance suite rather than the parity test
...
## 12. Fixtures and validation

The behaviors above are pinned by these artifacts. Any reimplementation is
...
The suite in `tests/conformance/` imports the implementation named by the
`PI_WEB_SEARCH_IMPL` environment variable (default: the real entry point) and
asserts the behaviors above. To validate a reimplementation:

```sh
PI_WEB_SEARCH_IMPL=/path/to/other/index.ts npm run conformance
```

See [RECREATION.md](./RECREATION.md) for the step-by-step reimplementation
guide.
